...
if ($d_prvl['c_praktik'] == "Y") {
    ...
    if (
        ...
        if ($d_jenis_jurusan['id_jurusan_pdd_jenis'] == 1) {
            ...
            $array[5]  = [$id_praktik, date('Y-m-d'), "PEMAKAIAN KEKAYAAN DAERAH", "Kelas", 30000, "per-siswa / periode", 0, $_POST['jumlah'],  0];
            ...
            $array[0] = [$id_praktik, date('Y-m-d'), "BIAYA ADMINISTRASI", "Institusional Fee", 20000, "per-siswa / periode", 1, $_POST['jumlah'],  1 * (int)$_POST['jumlah'] * 20000];
            $array[1] = [$id_praktik, date('Y-m-d'), "BIAYA ADMINISTRASI", "Management Fee", 20000, "per-siswa / periode", 1, $_POST['jumlah'],  1 * (int)$_POST['jumlah'] * 20000];
            $array[2]  = [$id_praktik, date('Y-m-d'), "BIAYA HABIS PAKAI", "(Handrub,tisue,sabun)", 20000, "per-siswa / periode", 1, $_POST['jumlah'],  1 * (int)$_POST['jumlah'] * 20000];
            $array[3]  = [$id_praktik, date('Y-m-d'), "BIAYA OVERHEAD OPERASIONAL", "Orientasi", 75000, "per-siswa / kali", 1, 1,  1 * (int)$_POST['jumlah'] * 75000];
            $array[4]  = [$id_praktik, date('Y-m-d'), "BIAYA OVERHEAD OPERASIONAL", "Name Tag", 10000, "per-siswa", 1, $_POST['jumlah'],  1 * (int)$_POST['jumlah'] * 10000];
            $array[5]  = [$id_praktik, date('Y-m-d'), "PEMAKAIAN KEKAYAAN DAERAH", "Aula", 1000000, "per-periode", 1, 1,  1000000];
            $array[6]  = [$id_praktik, date('Y-m-d'), "BIAYA PEMBELAJARAN", "BST / Bimbingan", 75000, "per-siswa / minggu", 0, $_POST['jumlah'],  0];
